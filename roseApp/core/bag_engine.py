...
class BagAnalysisEngine:
    ...
    async def prune_bag(self, input_path: str,
                       output_path: str,
                       start_time: Optional[float] = None,
                       end_time: Optional[float] = None,
        ...
        """
        Prune bag file by time range and topics
        
        Args:
            input_path: Input bag file path
            output_path: Output bag file path
            start_time: Start time for pruning (timestamp)
            end_time: End time for pruning (timestamp)
            topics: List of topics to include
            compression: Compression type
        ...
        """
        try:
            # Get best parser for the task
            parser = await self.parser_manager.get_best_parser(input_path)
            
            # Use IO manager for efficient processing
            result = await self.io_manager.prune_bag(
                input_path=input_path,
                output_path=output_path,
            ...
            self.logger.error(f"Error pruning bag {input_path}: {e}")
            raise
    
    async def extract_field_data(self, bag_path: str,
                               field_path: str,
                               time_range: Optional[Tuple[float, float]] = None,
                               progress_callback: Optional[callable] = None) -> Dict[str, Any]:
        """
        Extract field data from bag for plotting
        
        Args:
            bag_path: Path to bag file
            field_path: Field path in format 'topic.field'
        ...
        try:
            ...
            if '.' not in field_path:
                raise ValueError("Field path must be in format 'topic.field'")
            
            topic, field = field_path.split('.', 1)
            
            # Get best parser for the task
            parser = await self.parser_manager.get_best_parser(bag_path)
            
            # Use IO manager for efficient data extraction
            result = await self.io_manager.extract_field_data(
                bag_path=bag_path,
                topic=topic,
                field=field,
            ...
            self.logger.error(f"Error extracting field data from {bag_path}: {e}")
            raise
