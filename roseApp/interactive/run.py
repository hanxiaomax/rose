...
def interactive():
    ...
    This mode provides:
    - REPL interface with slash commands (/load, /extract, /status, etc.)
    - Background task execution with real-time feedback
    - Session state management with workspace tracking
    - Intelligent auto-completion and command suggestions
    - Context-aware help and guidance
    - @ references for cached bags (@test.bag)
    
    Key Features:
    - Slash commands: /load, /extract, /inspect, /status, etc.
    ...
    - @ symbol for cached bags: @test.bag resolves to full path
    - Shell commands: !ls -la for native shell access
    
    Examples:
        > /load data.bag
        > /extract @input output.bag -t /camera/image
        > /inspect @test --topics
        > /status
        > /help load
    """
    # Use the new refactored runner
